 ...
 */
public class SecretAsymmetricPerson {
	 ...
	 */
	public SecretAsymmetricPerson(String name, String keyType) throws Exception {
		this.name = name;
	 ...
	 */
	public void sendMessage(SecretAsymmetricPerson to, String message) throws Exception {
		System.out.println(name + " sends message to "+to.getName());
		cipher.init(Cipher.ENCRYPT_MODE, to.getPublicKey());
		byte[] encrypted = cipher.doFinal(message.getBytes());
		System.out.println("Encrypted message ("+encrypted.length+" bytes): "+Hex.toHexString(encrypted));
