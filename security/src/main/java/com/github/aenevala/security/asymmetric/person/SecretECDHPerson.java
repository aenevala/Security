...
/**
 * Person class that uses Elliptic Curve Diffie Hellman key agreement.
 * Sending a message is using ephmeral key pair for each message.
 * Only receiver's static key is needed to be known in advance.
 */
public class SecretECDHPerson {
	...
	public SecretECDHPerson(String name) throws Exception {
		...
		ka = KeyAgreement.getInstance(X9ObjectIdentifiers.dhSinglePass_stdDH_sha1kdf_scheme.getId());
	}
	 ...
	 * @param publicKey ephmeral public key used in key agreement
	 * @throws Exception if something goes wrong
	 */
	public void receiveMessage(SecretECDHPerson from, byte[] encrypted, PublicKey publicKey) throws Exception {
	 ...
	 */
	public void sendMessage(SecretECDHPerson to, String message) throws Exception {
